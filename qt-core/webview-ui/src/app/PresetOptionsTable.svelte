...
<script lang="ts">
  ...
  import { createCustomPreset, deleteCustomPreset as deleteCurrentPreset } from './viewlogic.svelte';
...
{#if steps}
  ...
  <div class="w-full flex flex-row justify-end mt-2">
    <Button class="qt-button z-200" on:click={onDeleteClicked}>
      Delete
    </Button>
    <Button class="qt-button z-200" on:click={onSaveClicked}>
      Save
    </Button>
  </div>
